...
export default function Component({ mnemonic = "your mnemonic phrase here" }: { mnemonic?: string }) {
  ...
  return (
    <div className="p-4 bg-gray-900 text-gray-300 shadow-lg rounded-lg sm:p-6">
      <Button
      ...
      {publicKeys.map((publicKey, index) => (
        <div key={index} className="bg-gray-800 shadow-md rounded-md mb-4 p-4">
          <div className="mb-2">
            <p className="text-[#c0cad4] font-medium text-sm sm:text-base break-all">
              Public Key {index + 1}: {publicKey}
            </p>
          </div>
          <div className="mb-2">
            <p className="text-gray-400 font-medium text-sm sm:text-base">Private Key {index + 1}:</p>
            <div className="bg-gray-800 py-1 px-2 rounded-md flex items-center justify-between mt-1">
              <p
